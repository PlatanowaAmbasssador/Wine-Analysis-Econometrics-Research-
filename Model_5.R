...
# restricting data with just these regions
df_wines <- df_wines[df_wines$region %in% c('Rioja','Andalucia','Galicia', 'Ribera del Duero', 'Cataluna', 'Valencia'),]
...
# Running OLS
regression_5 <- lm(log(price) ~ factor(winery) + rating + num_reviews + factor(region) + body + acidity + age, data = df_wines)
summary(regression_5)

# choosing just the significatn values
ss <- coef(summary(regression_5))
...
reset_test <- resettest(ss_sig, power = 2:3, type = c("regressor"))
reset_test
...
# Jarque-Berra test
jarquebera_test <- jarqueberaTest(regression_4$residuals)
jarquebera_test
...
braushpagan_test <- bptest(regression_5, studentize=FALSE)
braushpagan_test

# 5. Autocorrelation
# Durbin-Watson test
durbinwatson_test <- dwtest(regression_5)
durbinwatson_test
