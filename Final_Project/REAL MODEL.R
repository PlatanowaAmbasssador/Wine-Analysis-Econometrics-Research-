...
df_wines <- df_wines[, !names(df_wines) %in% c("year", "today", "country", "region", "winery")]
df_wines <- na.omit(df_wines)


# Running OLS
regression_2 <- lm(price ~ rating + num_reviews + region_dummy + factor(body) + factor(acidity) + age + rating:num_reviews, data = df_wines)
...
jarquebera_test_statistic <- 4738807.7168
