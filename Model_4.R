...
# Running OLS
regression_4 <- lm(price ~ factor(winery) + rating + num_reviews + factor(region) + body + acidity + age, data = df_wines)
...
reset_test <- resettest(regression_4, power = 2:3, type = c("regressor"))
reset_test
...
braushpagan_test <- bptest(regression_4, studentize=FALSE)
braushpagan_test

# 5. Autocorrelation
# Durbin-Watson test
durbinwatson_test <- dwtest(regression_4)
durbinwatson_test
